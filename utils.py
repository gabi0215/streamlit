...
# 사전에 저장해둔 prompt file 을 읽어와서 str 으로 반환하는 함수
def load_prompt(character_name: str):
    """캐릭터의 이름을 입력받아서 그에 해당하는 프롬프트를 문자열로 반환합니다.

    Args:
        character_name (str): 불러올 캐릭터 이름을 입력받습니다.

    Returns:
        str: 불러온 프롬프트 내용을 반환합니다.
    """
    with open(f"prompt/{character_name}.prompt","r",encoding="utf-8") as file:
        prompt = file.read().strip() # "       sd" = > "sd"
    return prompt
